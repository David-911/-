...
class CreateButton(object):
    def __init__(self,master=None,canvas=None,x1=None,y1=None,x2=None,y2=None,str1=None,fontsize=30):
        ...
        self.SetBuuton()
        bo10 = Button(self.root,  bd=4,bg='gold',font=('楷体', 30),  text='开始游戏', command=self.start_game)
        bo10.place(width=200, height=50, x=600, y=10)

    def start_game(self):
        game_window = tk.Toplevel(self.root)
        game = FishGame(game_window)
...
class FishGame:#游戏界面
    def __init__(self, master):
        ...
        self.master.title("功德游戏")

        # 初始化 pygame
        pygame.mixer.init()
        self.hit_sound = pygame.mixer.Sound("muyu.mp3")

        # 创建画布并设置背景图片
        self.canvas = tk.Canvas(self.master, width=800, height=500, bd=0, highlightthickness=0)
        self.canvas.pack(side=tk.TOP)
        self.bg_image = Image.open("fozu.jpg")
        self.bg_image = self.bg_image.resize((800, 500))
        self.photo = ImageTk.PhotoImage(self.bg_image)
        self.canvas.create_image(0, 0, anchor=tk.NW, image=self.photo)

        # 创建木鱼图标并初始化位置
        self.fish_image = Image.open("gongde.jpg")
        self.fish_image = self.fish_image.resize((80, 50))
        self.fish_list = []
        self.fish_photo = ImageTk.PhotoImage(self.fish_image)

        # 注册事件监听函数
        self.master.bind("<space>", self.catch_fish)
        self.update_fish_location()

    def update_fish_location(self):
        # 随机生成木鱼
        if random.random() < 0.05:
            pos = (random.randint(0, 750), 0)
            fish_id = self.canvas.create_image(pos[0], pos[1], anchor=tk.CENTER, image=self.fish_photo)
            self.fish_list.append({"id": fish_id, "pos": pos})
        ...
        for i in range(len(self.fish_list)):
            ...
            if fish_id is not None:
                self.canvas.coords(fish_id, pos[0], pos[1])
                fish["pos"] = (pos[0], pos[1] + 5)
            # 判断是否到达下边缘
            if pos[1] >= 535 and pos[1] <= 565:
                self.canvas.itemconfig(fish_id, fill="red")
            if pos[1] > 570:
                self.fish_list[i] = {"id": None, "pos": (0, 0)}
                self.canvas.delete(fish_id)

        # 继续移动木鱼
        self.master.after(50, self.update_fish_location)

    def catch_fish(self, event):
        found = False
        for fish in self.fish_list:
            if fish["id"] is not None:
                x, y = self.canvas.coords(fish["id"])
                if y > 460 and y < 540:
                    self.canvas.delete(fish["id"])
                    fish["id"] = None
                    self.hit_sound.play()  # 播放敲木鱼音效
                    found = True
                    break

        # 显示结果
        text_id = self.canvas.create_text(400, 550, text="", font=("Arial", 24))
        if found:
            result_text = "Success! Glad tidings!"
            self.canvas.itemconfig(text_id, text=result_text, fill="blue")
        else:
            result_text = "Oops! Bad karma!"
            self.canvas.itemconfig(text_id, text=result_text, fill="red")
...
class LoginPage(object):
    ...
    def sign(self):#注册函数
        ...
        new_name.set('e.g.npucs2022666666')
